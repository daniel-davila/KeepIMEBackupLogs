...
if (Test-Path $IMEPath)
{
	...
	{
		Copy-Item $MyInvocation.MyCommand.Definition -Destination $IMEPath -ErrorAction SilentlyContinue
		$TaskAction = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-ExecutionPolicy Bypass -NoProfile -File $($IMEPath)\$($MyInvocation.MyCommand.Name)"
...
{
	Write-Host "IME Log path doesn't exist. Exiting."; Exit
}
